...
app.delete('/api/expenses/:id', async (req, res) => {
  try {
    ...
    if (exp && exp.expense_type === 'caixa') {
      await queryRun(
        "DELETE FROM cash_movements WHERE store_id = $1 AND type = 'saida' AND value = $2 AND description LIKE $3 AND created_at >= (NOW() AT TIME ZONE 'America/Sao_Paulo')::date + interval '3 hours'",
