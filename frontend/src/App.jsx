          ...
          {showQuickAdd&&<div style={{marginTop:6,background:C.s2,borderRadius:10,padding:10,border:`1px solid rgba(0,230,118,.2)`,animation:"fadeIn .2s ease"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:5}}>
              ...
              <div style={{display:"flex",gap:5}}>
                <input style={{...S.inp,fontSize:12,padding:"6px 10px",flex:1}} type="date" title="Aniversário (cashback em dobro no dia)" value={qc.birthdate} onChange={e=>setQc(q=>({...q,birthdate:e.target.value}))}/>
...
function CRMModule({customers,setCustomers,storeSales,showToast}){
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
        <div style={{width:300,maxWidth:"100%",display:"flex",flexDirection:"column",gap:6}}>{filtered.map(c=><button key={c.id} style={{...S.crmCard,...(sel?.id===c.id?{borderColor:C.gold,background:"rgba(255,215,64,.04)"}:{})}} onClick={()=>setSel(c)}><div style={S.avatar}>{c.name.charAt(0)}</div><div style={{flex:1,overflow:"hidden"}}><div style={{fontWeight:700,fontSize:13}}>{c.name}</div><div style={{fontSize:11,color:C.dim}}>{c.phone}</div><div style={{display:"flex",gap:3,marginTop:3,flexWrap:"wrap"}}>{c.tags.map(t=><span key={t} style={S.tag}>{t}</span>)}</div></div><div style={{textAlign:"right"}}><div style={{fontWeight:700,color:C.grn,fontSize:12}}>{fmt(c.totalSpent)}</div><div style={{fontSize:9,fontWeight:800,color:(c.cpf||"").length>=11?(c.tier==="DIAMOND"?C.blu:C.gold):C.dim}}>{(c.cpf||"").length>=11?("🖤 "+(c.tier==="GOLD"?"GOLD":c.tier==="DIAMOND"?"DIAMOND":"BLACK")):"sem CPF"}</div></div></button>)}</div>
        {sel?<div style={{flex:1,minWidth:280,...S.card,animation:"fadeIn .3s ease"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(150px,1fr))",gap:10,marginBottom:14}}><div><span style={{fontSize:10,color:C.dim}}>Telefone</span><div>{sel.phone}</div></div><div><span style={{fontSize:10,color:C.dim}}>E-mail</span><div>{sel.email||"-"}</div></div><div><span style={{fontSize:10,color:C.dim}}>CPF</span><div>{sel.cpf||"-"}</div></div><div><span style={{fontSize:10,color:C.dim}}>Total Gasto</span><div style={{color:C.grn,fontWeight:700}}>{fmt(sel.totalSpent)}</div></div><div><span style={{fontSize:10,color:C.dim}}>Cliente Black</span><div style={{color:(sel.cpf||"").length>=11?(sel.tier==="DIAMOND"?C.blu:C.gold):C.dim,fontWeight:700}}>{(sel.cpf||"").length>=11?("🖤 "+(sel.tier==="GOLD"?"GOLD":sel.tier==="DIAMOND"?"DIAMOND":"BLACK")):"sem CPF"}</div></div><div><span style={{fontSize:10,color:C.dim}}>Visitas</span><div>{sel.visits}</div></div></div>
...
function WhatsAppModule({customers}){
  ...
  return(
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:10}}>{filtered.map(c=><div key={c.id} style={S.card}><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}><div style={S.avatar}>{c.name.charAt(0)}</div><div style={{flex:1}}><div style={{fontWeight:700,fontSize:13}}>{c.name}</div><div style={{fontSize:11,color:C.dim}}>{c.phone}</div></div></div><div style={{fontSize:11,color:C.dim,marginBottom:8}}>Gasto: {fmt(c.totalSpent)} • {(c.cpf||"").length>=11?("🖤 "+(c.tier==="GOLD"?"GOLD":c.tier==="DIAMOND"?"DIAMOND":"BLACK")):"sem CPF"}</div><button onClick={()=>openWa(c)} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"center",gap:6,padding:"8px",borderRadius:8,border:"none",background:"#25D366",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>📱 Enviar Mensagem</button></div>)}</div>
    </div>
  );
}
